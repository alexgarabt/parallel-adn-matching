...
void increment_matches( int pat, unsigned long *pat_found, long *pat_length, int *seq_matches ) {
	...
	#pragma omp parallel for default(none) shared(pat_length, seq_matches, pat_found, pat) private(ind)
...
int main(int argc, char *argv[]) {
	...
	case 'A':
		...
		for( ind=pat_rng_num; ind<pat_number; ind++ ) pat_type[ind] = PAT_TYPE_SAMP;
		break;
	case 'B':
		for( ind=0; ind<pat_samp_num; ind++ ) pat_type[ind] = PAT_TYPE_SAMP;
		for( ind=pat_samp_num; ind<pat_number; ind++ ) pat_type[ind] = PAT_TYPE_RNG;
		break;
	default:
		if ( pat_rng_num == 0 ) {
 ...
 */
	...
	/*
	 * Se podría paralelizar con un parallel for simple, pero habría que ver si aplica mucha mejora o no.
	 * Primero envíar el codigo al tablón sin paralelizar y luego enviarlo paralelizado y comparar.
	 */

	/* 4. Initialize ancillary structures */
	#pragma omp parallel for shared(pat_found) 
	for( ind=0; ind<pat_number; ind++) {
		pat_found[ind] = NOT_FOUND;
	}
	#pragma omp parallel for shared(seq_matches, seq_longest)
	for( ind=0; ind<seq_length; ind++) {
		seq_matches[ind] = 0;
		seq_longest[ind] = 0;
	}
	...
	unsigned long start;
	unsigned long pat;
	#pragma omp parallel for default(none) shared(pat_matches, seq_matches, seq_length, pat_length, pat_found, sequence, pattern, pat_number) private(pat, start, ind) 
	...
	#pragma omp parallel for default(none) shared(seq_length, seq_longest, pat_found, pat_length, pat_number) private(ind, pat)
	for( ind=0; ind < seq_length; ind++) {
		int max_length = 0;	
		for( pat=0; pat<pat_number; pat++ ) {
			if ( pat_found[pat] != NOT_FOUND )
	...
	#pragma omp parallel for default(none) shared(pat_number, pat_found ) private(ind) reduction(+:checksum_found)
	for( ind=0; ind < pat_number; ind++) {
	...
	#pragma omp parallel for default(none) shared ( seq_longest, seq_length, seq_matches) reduction(+:checksum_matches) reduction(+:checksum_longest)
	for( ind=0; ind < seq_length; ind++) {
